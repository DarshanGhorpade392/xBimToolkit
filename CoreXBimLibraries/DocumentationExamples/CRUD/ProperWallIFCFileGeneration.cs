...
namespace HelloWall
{
    class ProperWallIFCFileGeneration
    {
        /// <summary>
                ...
                if (model != null)
                {
                    ...
                    if (wall != null)
                    {
                        try
                        {
                            ...
                            //write the Ifc File
                            model.SaveAs("HelloWallIfc4.ifc", IfcStorageTypeExtensions.StorageType(".ifc"));
        ...
        static private IfcWallStandardCase CreateWall(IfcStore model, double length, double width, double height)
        {
            ...
            using (var txn = model.BeginTransaction("Create Wall"))
            {
                ...
                body.Depth = height; 
                body.SweptArea = rectProf; // The surface defining the area to be swept. It is given as a profile definition within the xy plane of the position coordinate system
                body.ExtrudedDirection = model.Instances.New<IfcDirection>(); // direction of the extrusion
                body.ExtrudedDirection.SetXYZ(0, 0, 1);     // z direction

                //parameters to insert the geometry in the model
                var origin = model.Instances.New<IfcCartesianPoint>();
                origin.SetXYZ(0, 0, 0);
                body.Position = model.Instances.New<IfcAxis2Placement3D>(); // provides location and orientations to place items in a three-dimensional space
                body.Position.Location = origin;

                //Create a Definition shape to hold the geometry
                var shape = model.Instances.New<IfcShapeRepresentation>();  // concept of a particular geometric representation of a product or a product component within a specific geometric representation context

                /*
                 * IfcGeometricRepresentationContext
                 * context that applies to several shape representations of products within a project.
                 * type of the context in which the shape representation is defined,
                 * numeric precision applicable to the geometric representation items defined in this context
                 */
                ...
                var ifcMaterialLayerSetUsage = model.Instances.New<IfcMaterialLayerSetUsage>();  // determines the usage of IfcMaterialLayerSet in terms of its location and orientation relative to the associated element geometry
                var ifcMaterialLayerSet = model.Instances.New<IfcMaterialLayerSet>(); // designation by which materials of an element constructed of a number of material layers is known and through which the relative positioning of individual layers can be expressed.
                var ifcMaterialLayer = model.Instances.New<IfcMaterialLayer>(); // single and identifiable part of an element which is constructed of a number of layers (one or more). Each IfcMaterialLayer has a constant thickness and is located relative to the referencing IfcMaterialLayerSet along the material layer set base (MlsBase).
                ...
                var material = model.Instances.New<IfcMaterial>(); // IfcMaterial is a homogeneous or inhomogeneous substance that can be used to form elements (physical products or their components).
                material.Name = "some material";
                var ifcRelAssociatesMaterial = model.Instances.New<IfcRelAssociatesMaterial>(); // objectified relationship between a material definition and elements or element types to which this material definition applies
                ...
                var ifcPresentationLayerAssignment = model.Instances.New<IfcPresentationLayerAssignment>(); // provides the layer name (and optionally a description and an identifier) for a collection of geometric representation items
        ...
        private static void CreateSimpleProperty(IfcStore model, IfcWallStandardCase wall)
        {
            ...
            {
                dr.Name = "Document";
                dr.Location = "C://Users//Darshan.Ghorpade//Documents//TheDoc.Txt";
            });
